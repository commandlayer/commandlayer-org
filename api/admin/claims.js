...
module.exports = async function handler(req, res) {
  ...
  const result = await db.query(
    `select
      ...
      cr.payment_status,
      cr.activation_mode,
      cr.tenant_signer_record_status,
      cr.managed_ens_publication_status,
      cr.tenant_proof_status,
      cr.created_at,
      cr.paid_at,
  ...
  const claims = result.rows.map((row) => ({
    ...
    tenantSignerRecordStatus: row.tenant_signer_record_status,
    managedEnsPublicationStatus: row.managed_ens_publication_status,
    tenantProofStatus: row.tenant_proof_status,
